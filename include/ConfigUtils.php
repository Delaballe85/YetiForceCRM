...
class AppConfig
{
	...
	public static function module()
	{
		...
		$fileName = ROOT_DIRECTORY . 'config/modules/' . $module . '.php';
	...
	public static function developer($key, $defvalue = false)
	{
		if (empty(self::$developer)) {
			require_once ROOT_DIRECTORY . 'config/developer.php';
	...
	public static function security($key, $defvalue = false)
	{
		if (empty(self::$security)) {
			require_once ROOT_DIRECTORY . 'config/security.php';
	...
	public static function securityKeys($key, $defvalue = false)
	{
		if (empty(self::$securityKeys)) {
			require_once ROOT_DIRECTORY . 'config/secret_keys.php';
	...
	public static function relation($key, $defvalue = false)
	{
		if (empty(self::$relation)) {
			require_once ROOT_DIRECTORY . 'config/relation.php';
	...
	public static function sounds()
	{
		if (empty(self::$sounds)) {
			require_once ROOT_DIRECTORY . 'config/sounds.php';
	...
	public static function search($key, $defvalue = false)
	{
		if (empty(self::$search)) {
			require_once ROOT_DIRECTORY . 'config/search.php';
...
session_save_path(ROOT_DIRECTORY . 'cache' . DIRECTORY_SEPARATOR . 'session');
// Change of logs directory with PHP errors
AppConfig::iniSet('error_log', ROOT_DIRECTORY . 'cache' . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'phpError.log');
